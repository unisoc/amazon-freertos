...
int wifi_cmd_load_ini(const u8_t *data, u32_t len, u8_t sec_num)
{
	...
	cmd = UWP_MEM_ALLOC(cmd_len);//cmd = malloc(cmd_len);
	if (!cmd) {
		LOG_ERR("Cmd is null");
		return -ENOMEM;
	}
...
int wifi_cmd_scan(struct wifi_device *wifi_dev, struct wifi_drv_scan_params *params)
{
	...
	ret = wifi_cmd_send(WIFI_CMD_SCAN, (char *)cmd,
			    cmd_len, NULL, NULL);
	//DUMP_DATA((char *)cmd,0x44);
	if (ret) {
		LOG_ERR("Scan send cmd fail,ret=%d",ret);
		UWP_MEM_FREE(cmd);//free(cmd);
		return ret;
	}
		     ...
		     struct wifi_drv_connect_params *params)
{
	...
	LOG_DBG("wifi_cmd_connect,ssid=%s,pwd=%s",params->ssid,params->psk);
    ...
    UWPEventGroupWaitBits(cmd_sem, CONNECTED_BIT | DISCONNECTED_BIT, pdTRUE, pdFALSE, UWP_DELAY_FOREVER);

    if (wifi_dev->connected == true) {
        return UWP_OK;
    } else {
    	return UWP_FAIL;
    }
...
int wifi_cmd_disconnect(struct wifi_device *wifi_dev)
{
    ...
    UWPEventGroupWaitBits(cmd_sem, DISCONNECTED_BIT, pdTRUE, pdFALSE, UWP_DELAY_FOREVER);
	return 0;
}
/*success,ret0*/
...
/*
	if (wifi_dev->scan_result_cb) {
		wifi_dev->scan_result_cb(event->status, NULL);
		wifi_dev->scan_result_cb = NULL;
	} else {
		LOG_WRN("No scan_result callback.");
	}
*/
    UWPEventGroupSetBits(cmd_sem, SCAN_DONE_BIT);
	return 0;
}
/*success,ret0*/
static int wifi_evt_connect(struct wifi_device *wifi_dev, char *data, int len)
{
	...
	LOG_DBG("wifi_evt_connect=%d\r\n",event->status);
	if (!event->status) {
		wifi_dev->connected = true;
	}

    UWPEventGroupClearBits(cmd_sem, DISCONNECTED_BIT);
    UWPEventGroupSetBits(cmd_sem, CONNECTED_BIT);
		...
		char *data, int len)
{
	...
	LOG_DBG("wifi_evt_disconnect=%d\r\n",event->reason_code);
	wifi_dev->connected = false;

    UWPEventGroupClearBits(cmd_sem, CONNECTED_BIT);
    UWPEventGroupSetBits(cmd_sem, DISCONNECTED_BIT);
...
int wifi_cmdevt_process(struct wifi_priv *priv, char *data, int len)
{
	...
	LOG_DBG("wifi_cmdevt_process,hdr_resp=%d,hdr_status=%d,len=%d\r\n",hdr->response, hdr->status, len);
	...
	if (hdr->response == 1) {
	    ...
	    //OS_SEM_GIVE(cmd_sem);//xEventGroupSetBits(cmd_sem, CMD_DONE_BIT);//k_sem_release(cmd_sem);
		UWPEventGroupSetBits(cmd_sem, CMD_DONE_BIT);
	...
	LOG_ERR("Receive event type 0x%x.", hdr->type);
...
{
    LOG_DBG("wifi_cmdevt_init,enter\r\n");
	if(cmd_sem == NULL)
	{
		//OS_SEM_INIT(cmd_sem, 1, 0);//cmd_sem = xEventGroupCreate();//cmd_sem = uwp_sem_create(1, 0);
		cmd_sem = UWPxEventGroupCreate();
	}
	else
	{
		LOG_ERR("wifi cmd sem initilized");
		return -1;
	}
	if(cmd_sem == NULL){
